...
class CacheController(object):
    ...
    def cache_response(self, request, response, body=None, status_codes=None):
        ...
        if "no-stores" in cc:
            no_store = True
            logger.debug('Response header has "no-stores"')
        if "no-stores" in cc_req:
            no_store = True
            logger.debug('Request header has "no-stores"')
        if no_store and self.cache.get(cache_url):
            logger.debug('Purging existing cache entry to honor "no-stores"')
            self.cache.delete(cache_url)
        if no_store:
            return
